        ...
        <tbody>
            @foreach ($maintenances as $maintenance)
                <tr>
                    ...
                    <td>
                        @can('service.edit')
                        <button class="btn btn-warning btn-sm" data-bs-toggle="modal" data-bs-target="#editMaintenanceModal{{ $maintenance->id }}">Editar</button>
                        @endcan
                        @can('service.delete')
                        <form action="{{ route('service.maintenances.destroy', $maintenance->id) }}" method="POST" class="d-inline" onsubmit="return confirm('Deseja excluir?')">
                            @csrf @method('DELETE')
                ...
                @include('service.forms.maintenance', ['maintenance' => $maintenance, 'clients' => $clients])
            @endforeach
        </tbody>
    </table>
